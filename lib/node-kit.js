 ...
 * @param  {String} token
 */
Kit.prototype._parseToken = function(token) {
...
Kit.prototype._compileToken = function(token) {
    ...
    // If we've got a predicate, we're assigning a value to this variable
        ...
        var varName = keyword.substring(1);
...
Kit.prototype._handleImport = function(filePath) {
    ...
    // Clone properties
    var variables = clone(this._variables);
    var forbiddenPaths = this._forbiddenPaths.slice();

    // Recurse and compile
    var result = new Kit(filePath, variables, forbiddenPaths).toString();
    for (var p in variables) { this._variables[p] = variables[p]; }
    return result;
};
